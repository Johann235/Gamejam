...
INCLUDE_DIRS= C:\\raylib\\raylib\\src
...
.PHONY = all info clean lib run help
...
help:
	@echo `make` makes the main program
	@echo `make lib` makes the program as a Library
	@echo `make clear` removes all files made by this makefile
